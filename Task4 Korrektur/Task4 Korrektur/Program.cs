...
namespace Task4
{
...

    {
        ...
        static void Main(string[] args)
        {
            ...
            var text = JsonConvert.SerializeObject(businesses, Formatting.Indented);
            var desktop = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
            string filename = desktop + @"\businesses.json";
            Console.WriteLine("    {0} ", filename);
            File.WriteAllText(filename, text);
            Console.ReadLine();

            var textFromFile = File.ReadAllText(filename);
            var businessesFromFile = JsonConvert.DeserializeObject<Business[]>(textFromFile, settings);
            foreach (var x in businessesFromFile) Console.WriteLine($"{x.Address} {x.Name}");
        }
